...
JSONForm.fieldTypes['image'] = {
    template: '<div><img class="field-image-preview img-thumbnail <% if(!value) { %>image-not-chosen<% } %>" style="cursor: pointer;" width="150" src="<% if(value) {%><%= window.asset(escape(value)) %><% } else{ %>' + chooseImagePlaceholder + '<% } %>"></div><br><input type="text" ' +
    ...
    onInsert: function (e, node) {
        ...
        formGroup.find('input').on('change', function(){
            ...
            previewImgElm.attr('src', window.asset(val));
            if(val) {
