<?php
class Helpers {
	public static function prettyTime($date, $includeTime = true) {
	...
	public static function getUrlWithQuery($queryArray = array(), $baseUrl = ''){
		$baseUrl = !empty($baseUrl) ? $baseUrl : Request::url();
		//die(var_dump($queryArray));
		return $baseUrl . '?' . http_build_query(array_merge(Input::get(), $queryArray));
	}
    ...
    public static function isValidUrl($url) {
        $url = Helpers::urlWithHttp($url);
        if(filter_var($url, FILTER_VALIDATE_URL) === FALSE)
            return false;

        //filter_var allows single words like "hbo" to be validated as url. so check that specifically.
        if(!strpos($url, '.')) {
            return false;
        }
        return true;
    ...
    public static function urlWithHttp($url) {
